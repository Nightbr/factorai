...
const PROJECT_SELECT: &str = "SELECT p.id, p.real_path, p.display_name, p.missing,
...
///
/// **Flat on purpose, and not the sidebar's query.** The sidebar reads
...
pub fn list_projects_in(conn: &Connection) -> AppResult<Vec<Project>> {
	let sql = format!("{PROJECT_SELECT} ORDER BY p.display_name COLLATE NOCASE");
...
///
/// `display_name` is left alone on conflict, and the project keeps the sidebar
/// row it already has — re-adding a folder must not silently rename it or move
/// it back to the top.
#[tauri::command]
...
pub fn add_project_in(db: &Db, path: &str) -> AppResult<Project> {
	...
	db.with_mut(|conn| {
		...
		let row_project_id: String = tx.query_row(
			"SELECT id FROM projects WHERE real_path = ?1",
			params![real_path],
			|r| r.get(0),
		)?;
