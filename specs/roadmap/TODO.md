...
## 46. Pin a session so it sits at the top of the list

Asked for 2026-09-01: a session you mark stays at the top of its project's list, in the sidebar
and on the project page. The sessions you keep coming back to — the long-running one, the one
holding the branch you are reviewing — sink under whatever you touched most recently, and today
the only way to keep one to hand is to leave a tab open.

**Item 28 retired the project pin for an ordinal, and that is not an argument against this one.**
Migration `0011_project_sort_order.sql` says why the project pin lost: it was "a one-bit
approximation of an ordering" over a list of ten stable rows you can meaningfully drag into the
order you want. A session list is the opposite — dozens of rows, sorted by a recency that changes
under you every turn, with rows arriving and being reaped by the indexer. There is no hand order
to preserve, so one bit is the whole of the requirement: *exempt this row from recency*. Say that
in the migration comment, because the next reader will arrive holding 0011.

- [ ] **Where the bit lives.** A `session_pins` sidecar table — `session_id` primary key, and a
      `pinned_at` worth keeping so several pins order among themselves by when they were made
      rather than arbitrarily. **No foreign key to `sessions`**, which is 0007's lesson and the
      shape `session_worktrees` and `session_routines` already use: a session has no row until
      Claude has written a transcript and the watcher has indexed it, so a column on `sessions`
      cannot record a pin on the live rows `pendingSessions` draws (F6). Add the delete to the
      indexer's reap beside `del_wt` and `del_rt` — a pin on a transcript that is gone is not
      state worth keeping.
      A column on `sessions` would survive the indexer, incidentally: `index_one`'s upsert names
      the columns it overwrites. That is not the reason to avoid it; the pending rows are.
- [ ] **Commands and the wire.** `set_session_pinned(session_id, pinned)` in `commands/sessions.rs`,
      and `pinned: bool` on `SessionSummary` — joined in `list_sessions_in` like `w.path` and
      `sr.routine_id`, for the same stated reason: every row that draws it needs it, on first
      paint, before any event has had a cause to fire. Mirror the field in
      `packages/types/src/index.ts`. Emit `sessions:changed` on the write so the sidebar and the
      project page both refetch through the path they already have.
- [ ] **Sidebar ordering, and the cap.** `orderSessions` in `SidebarProject.tsx` sorts live-PTY
      rows first, then by recency, then truncates to `SIDEBAR_SESSION_LIMIT` (10). A pin that can
      be truncated is not a pin, so pinned rows sort above everything and are never the rows the
      slice drops — this is the only real ordering decision in the item, and it settles as
      **pinned, then open, then recency**. The pure exported function is where the rules live and
      `SidebarProject.test.ts` is where they get asserted; no rendering needed.
- [ ] **The project page.** `routes/project.tsx` draws the backend's order through
      `groupSessions`, which nests and never sorts, on purpose. Pinning must not change that: put
      the pin in the backend `ORDER BY`, ahead of the parent-recency key, so a pinned parent takes
      its sub-agents with it and the renderer keeps its one job. A pinned *sub-agent* is a
      question — the honest answer is that the control does not appear on sub-agent rows, since
      they are not rows you navigate back into and the sidebar already filters them out.
- [ ] **The control.** `SessionRow` already has a context menu (delete, copy transcript path), so
      the pin is one `ContextMenuItem` above the separator, toggling label between Pin and Unpin.
      The pinned state also has to be visible on the row itself, or a list with two pins reads as
      a broken sort — a small mark in the same slot the routine and checkout marks use. Keyboard
      path required, as everywhere: the context menu is reachable from the focused row, which is
      the existing pattern rather than a new binding.
- [ ] **Update F2** (`specs/05-features.md` § "F2 — Session list") in the same commit as the
      command, per the `spec-and-adr-workflow` skill. F2 states the list order today; it becomes
      the contract for the three-key order above, and for pinning being per-session rather than
      per-project-view state.

**No ADR needed** unless the ordering question above is reopened — nothing here supersedes a
decision. Worth a line in 0011's spirit inside the new migration, though, so the two pins in this
codebase's history do not read as a reversal.

**Small, and a good candidate to land beside item 17** (renaming a session): same row, same menu,
same spec section — with the difference that a pin is entirely ours and asks nothing of ADR-0004,
which is why it can ship first.
