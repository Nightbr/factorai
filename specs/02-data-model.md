...
| indexed_bytes  | INTEGER    | the offset the indexer finished at, and where the next pass resumes (migration 0021, PERF-03). Everything whose event was taken, plus every whole line permanently skipped — **not** `file_size`, because a read can land inside a line Claude is still writing, and resuming past it would lose that event for good. 0 means "parse the whole file", which is what every pre-0021 row says. |
| title_kind     | TEXT       | which source `title` came from: `custom` (a `/rename`), `ai` (Claude's own) or `derived` (the first user message, or the id). A full parse settles precedence by reading the whole file; a tail parse sees only what was appended, so it has to know what it is up against — an `ai-title` in the tail must not undo an earlier `/rename`. NULL is a pre-0021 row and forces one full parse. |
